...
// NewRedisDriver 创建新的redis存储
func NewRedisDriver(size int, network, address, password string, database int) *RedisDriver {
	return &RedisDriver{
		client: redis.NewClient(&redis.Options{
			...
			Dialer: func(ctx context.Context, network, addr string) (net.Conn, error) {
				...
				if err != nil {
					logger.Log().Panicf("Redis 连接错误，%s", err)
					return nil, err
				}
...
// Set 存储值
func (store *RedisDriver) Set(key string, value interface{}, ttl int64) error {
	rc := store.client.Conn(store.ctx)
	defer func(rc *redis.Conn) {
		err := rc.Close()
		if err != nil {
			logger.Log().Errorf("Redis 关闭连接错误，%s", err)
		}
	}(rc)
...
// Get 取值
func (store *RedisDriver) Get(key string) (interface{}, bool) {
	rc := store.client.Conn(store.ctx)
	defer func(rc *redis.Conn) {
		err := rc.Close()
		if err != nil {
...
// Gets 批量取值
func (store *RedisDriver) Gets(keys []string, prefix string) (map[string]interface{}, []string) {
	rc := store.client.Conn(store.ctx)
	defer func(rc *redis.Conn) {
		err := rc.Close()
		if err != nil {
...
// Sets 批量设置值
func (store *RedisDriver) Sets(values map[string]interface{}, prefix string) error {
	rc := store.client.Conn(store.ctx)
	defer func(rc *redis.Conn) {
		err := rc.Close()
		if err != nil {
...
// Delete 删除给定的键
func (store *RedisDriver) Delete(key string) error {
	rc := store.client.Conn(store.ctx)
	defer func(rc *redis.Conn) {
		err := rc.Close()
		if err != nil {
...
// Deletes 批量删除给定的键
func (store *RedisDriver) Deletes(keys []string, prefix string) error {
	rc := store.client.Conn(store.ctx)
	defer func(rc *redis.Conn) {
		err := rc.Close()
		if err != nil {
...
// DeleteAll 批量所有键
func (store *RedisDriver) DeleteAll() error {
	rc := store.client.Conn(store.ctx)
	defer func(rc *redis.Conn) {
		err := rc.Close()
		if err != nil {
